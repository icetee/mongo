...
export default class Documents extends EtchComponent {
  ...
  expand() {
    return this.children.map((object, index) => {
...
class Objects extends EtchComponent {
  ...
  didCollapse(e) {
    ...
    if (e.target.parentElement.tagName === 'OBJECTS') {
      target = e.target.parentElement;
    } else if (e.target.parentElement.parentElement.tagName === 'OBJECTS') {
      target = e.target.parentElement.parentElement;
    }
...
class Arrays extends EtchComponent {
  render() {
    return <li
      attributes={{index: this.props.index}}
      className={`entry list-nested-item ${this.props.isExpanded ? 'expanded' : 'collapsed'}`}
    >
      <arrays>
      <ol className="entries list-tree">
        {
  ...
  expand() {
    return this.props.arrays.map((elem, index) => {
      return <Page
        ref={'page' + index}
        key=''
        value={elem}
  ...
  didCollapse(e) {
    ...
    if (e.target.parentElement.tagName === 'ARRAYS') {
      target = e.target.parentElement;
    } else if (e.target.parentElement.parentElement.tagName === 'ARRAYS') {
      target = e.target.parentElement.parentElement;
    }
...
class Page extends EtchComponent {
  render() {
    ...
    if (type === 'Object') {
      ...
      return <Arrays arrays={this.props.value} isExpanded={true} />;
    }
